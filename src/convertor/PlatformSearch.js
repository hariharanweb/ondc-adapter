...
export default class PlatformSearch {
  static async getPlatformResponse(ondcRequest) {
    ...
    const platformRequest = await PlatformSearch.generateRequest(ondcSearchJson);
  ...
  static async generateRequest(inputObject) {
    ...
    if (inputObject.productName === '' && inputObject.productId === '' && inputObject.categoryId === '' && inputObject.categoryName === '') {
      ...
      const platformConfigMapper = new PlatformConfigMapper('src/resource/category_mapping_config.csv');
      const categoryMapper = await platformConfigMapper.parseCSV();
      const platformCategoryId = categoryMapper
        .filter((x) => x.ondc_category === inputObject.categoryId)[0].platform_category;
      urlString = `${process.env.PLATFORM_BASE_URL}${process.env.PLATFORM_AUTH_QUERY_PARAM}&category=${platformCategoryId}`;
